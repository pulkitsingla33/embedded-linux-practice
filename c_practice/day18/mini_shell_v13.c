    ...
    while(1)
    {
        ...
        else
        {
            ...
            if((redirect_new != NULL) || (redirect_append != NULL) || (redirect_err != NULL))
            {
                ...
                {
                    *redirect_append = '\0';
                    char *file_name = redirect_append + 2;
                    while(*file_name == ' ')
                        file_name++;
                    redirect_fd = open(file_name, O_CREAT | O_WRONLY | O_APPEND, 0644);
                }
                else if(redirect_err != NULL)
                {
                    *redirect_err = '\0';
                    err_redirect_to_file = 1;
                    char *file_name = redirect_err + 2;
                    while(*file_name == ' ')
                        file_name++;
