...
int main()
{
    ...
    while(1)
    {
        ...
        if(strstr(command, "|||"))
        {
            fprintf(stderr, "mini-shell: syntax error near unexpected token '|'\n");
            continue;
        }
        ...
        else
        {
            ...
            while((redirect_new != NULL) && ((*(redirect_new + 1) == '>') || (*(redirect_new - 1) == '2')))
            {
                ...
                redirect_new = strchr(redirect_new, '>');
            }
            if((redirect_new != NULL) || (redirect_append != NULL) || (redirect_err != NULL))
            {
                ...
                if(redirect_append != NULL)
                {
                    ...
                    *redirect_append = '\0';
                    ...
                    if((append_filename == NULL) || (*append_filename == '\0'))
                    {
                        fprintf(stderr, "mini-shell: syntax error near unexpected token 'newline'\n");
                        continue;
                    }
                }
                if(redirect_err != NULL)
                {
                    *redirect_err = '\0';
                    ...
                    if((err_filename == NULL) || (*err_filename == '\0'))
                    {
                        fprintf(stderr, "mini-shell: syntax error near unexpected token 'newline'\n");
                        continue;
                    }
            ...
            if(redirect_input != NULL)
            {
                ...
                *redirect_input = '\0';
                ...
                int trailing_idx = strlen(input_file_name) - 1;
                while(trailing_idx >= 0 && input_file_name[trailing_idx] == ' ')
                {
                    trailing_idx--;
                }
                input_file_name[trailing_idx + 1] = '\0';
