...
#define MAX_CAPACITY (1024 * 1024 * 16)
...
#define SET_BUFFER_SIZE _IOW('a', 5, size_t)
...
static ssize_t device_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    space_left = cb.capacity - cb.count;
    if(space_left == 0)
    {
    ...
    chunk1 = (to_copy > (cb.capacity - cb.head)) ? (cb.capacity - cb.head) : to_copy;
    ...
    cb.head = (cb.head + to_copy) % cb.capacity;
    cb.count += to_copy;
...
static ssize_t device_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
    ...
    chunk1 = (to_read > (cb.capacity - cb.tail)) ? (cb.capacity - cb.tail) : to_read;
...
static long device_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case SET_BUFFER_SIZE:
            ...
            if(copy_from_user(&new_size, (size_t __user *)arg, sizeof(new_size)) != 0)
            {
                ret = -EFAULT;
                break;
            }
            ...
            if(new_size < cb.count)
            {
                printk(KERN_WARNING "mychardev: Resize Rejected. Unread data exceeds new size.\n");
                ret = -EINVAL;
                break;
            }
            ...
            chunk1 = (cb.count > (cb.capacity - cb.tail)) ? (cb.capacity - cb.tail) : cb.count;
            chunk2 = cb.count - chunk1;
            
            if(chunk1 > 0)
                memcpy(new_buffer, cb.buffer + cb.tail, chunk1);
            if(chunk2 > 0)
                memcpy(new_buffer + chunk1, cb.buffer, chunk2);
            ...
            cb.tail = 0;
            cb.head = (cb.count == cb.capacity) ? 0 : cb.count;

            printk(KERN_INFO "Buffer resized to %zu bytes\n", new_size);
            break;
        ...
        case RESET_DRIVER:
            ...
            memset(&stats, 0, sizeof(DriverStats));

            stored_value = 0;
            printk(KERN_INFO "mychardev: IOCTL RESET_DRIVER called\n");
            break;
...
static int __init chardev_init(void)
{
    ...
    cb.capacity = DEFAULT_CAPACITY;
    cb.buffer = kmalloc(cb.capacity, GFP_KERNEL);
    if(!cb.buffer)
    {
        printk(KERN_ALERT "Failed to allocate initial buffer\n");
        return -ENOMEM;
    }
